...
function findPokemonName() {
    const listButton = document.getElementsByClassName("listButton")
        for (let i = 0; i < listButton.length; i++) {
            const element = listButton[i].addEventListener("click",() => ((pokemonName = listButton[i].innerText.split("\n")[1]), getPokemonInfo()))
        }
}
    ...
    <div class="modal_content ${pokemonInfo.type}">
      ...
      <div class="modal_details">
        <div class="modal_img_container">
          <img class="modal_img" src="${pokemonInfo.photo}" alt="${pokemonInfo.name}">
        </div>
        ...
        <div class="details_container">
          <div class="modal_charac_container">
            ...
            <div class="modal_ability_container">
                ...
                <span>${pokemonInfo.abilities.map((ability) => `${ability}`.charAt(0).toUpperCase() + `${ability}`.slice(1)).join(", ")}</span>
            </div>
        ...
        <div class="stats_container">
            ...
            <div class="modal_stats_value">
                ${pokemonInfo.stats.map((stat) => `<span class="modal_stat_container">
                <span class="stat_value">${stat[1]}</span>
                <span class="stat_bar ${stat[1] >= 50 ? (stat[1] > 99 ? "gold" : "green") : "red"}"></span></span>`).join("")}
            </div>
          </div>
        </div>
      </div>
